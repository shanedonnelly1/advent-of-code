...
export const getMinCubeSetForCubeSets = (sets: CubeSet[]) => {
  const minCubeSet: CubeSet = sets.reduce(
    (minCubeSet, item) => {
      if (
        item.red &&
        (minCubeSet.red === 0 ||
          (!!minCubeSet.red && minCubeSet.red < item.red))
      ) {
        minCubeSet.red = item.red;
      }
      if (
        item.green &&
        (minCubeSet.green === 0 ||
          (!!minCubeSet.green && minCubeSet.green < item.green))
      ) {
        minCubeSet.green = item.green;
      }
      if (
        item.blue &&
        (minCubeSet.blue === 0 ||
          (!!minCubeSet.blue && minCubeSet.blue < item.blue))
      ) {
        minCubeSet.blue = item.blue;
      }
      return minCubeSet;
    },
...
export const powerOfMinSetForGameRowString = (line: string) => {
  const gameRow = parseGameRow(line);
  const cubeSet = getMinCubeSetForCubeSets(gameRow.sets);
  const red = cubeSet.red ?? 1;
  const green = cubeSet.green ?? 1;
  const blue = cubeSet.blue ?? 1;
  return Number(red) * Number(green) * Number(blue);
};

export const powerOfMinSetSumFromFile = async (
  file: PathLike,
  debug?: boolean
) => {
  var sum = 0;
  ...
  for await (const line of fileHandle.readLines()) {
    const rowValue = powerOfMinSetForGameRowString(line);
    sum += rowValue;
  }
  return sum;
};
